...
int checkid(char code[])
{
	...
	{
		if (isdigit(code[teller]))		{ geldig[0] = 1; }	else { geldig[0] = 0; break; }
	}
	if (code[0] == 80)					{ geldig[1] = 1; }	else { geldig[1] = 0; }
	if (code[12] == 0)					{ geldig[2] = 1; }	else { geldig[2] = 0; }

	som = code[0];

	for (teller = 1; teller <= 11; teller += 1)
	{
		som = som + (code[teller]-48);
	}
	
	uitkomst = som % 9;

	if (uitkomst == 1)					{ geldig[3] = 1; }	else { geldig[3] = 0; }
